...
The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project uses [Semantic Versioning](https://semver.org/). Each Scolta package versions independently; compatibility between packages is expressed by the dependency constraint an adapter declares for scolta-php rather than by matching version numbers.

## [Unreleased]

### Fixed
- **A quoted phrase query no longer returns documents that merely contain the words scattered apart (`score_results`, `batch_score_results`).** Pagefind has no phrase operator, so `"Boston massacre"` runs as `boston AND massacre`, and the forced-phrase mode only *boosted* adjacent matches through `phrase_adjacent_multiplier` — it never excluded non-adjacent ones. A Tulsa Race Massacre guide whose author is Carole **Boston** Weatherford therefore matched the quoted query. New `scoring::retain_forced_phrase_matches()` runs before scoring in both entry points: on a forced-phrase query with two or more meaningful terms, a result is dropped unless its title or excerpt (markup stripped) contains the raw quoted phrase literally, or its `locations` contain the meaningful terms' matched positions spanning less than the raw token count. The literal phrase and the span budget both come from the raw tokens inside the quotes, stop words included, so `"statue of liberty"` keeps exact-phrase documents (the filtered terms `statue liberty` would match neither the literal text nor the positions, which sit one index apart because "of" occupies a word index). Priority pages whose keywords match the query are exempt — they are pinned editorially. Results with too little location evidence (`locations` absent, empty, or holding fewer positions than there are meaningful terms) fail open and are kept — absence of data is not evidence of the phrase's absence. Unquoted and single-term queries are untouched. Known limitation: `locations` is a flat position array without term identity, so an adjacent reversed-order occurrence ("massacre Boston") still passes; the literal title/excerpt check is order-exact. No config, signature or `WASM_INTERFACE_VERSION` change. **[parity]** Quoted queries return fewer (correct) results on existing sites once adapters re-vendor the WASM build; `scolta.js` gains a matching change that skips AI query expansion for quoted queries so expansion cannot re-seed non-phrase documents.
...
- **Packages version independently, and `VERSIONING.md` now says so (`VERSIONING.md`, `CLAUDE.md`).** The written policy required a major version synchronized across all five packages, enforced in each PHP adapter by a `version-sync` CI job that failed unless the adapter's major equalled the major in its `tag1/scolta-php` constraint. That is a release obligation rather than a compatibility statement: a scolta-php 2.0 obliged a 2.0 of every adapter with nothing in them changed, and an adapter that did bump could not then ship against a supported 1.x library. The constraint an adapter declares for scolta-php already says which releases it runs against, and Composer enforces it on every install, so the rule restated the constraint in version numbers and charged four releases for it. Raised in review of [scolta-drupal#221](https://github.com/tag1consulting/scolta-drupal/pull/221); the three `version-sync` jobs are removed in [scolta-drupal#222](https://github.com/tag1consulting/scolta-drupal/pull/222), [scolta-laravel#126](https://github.com/tag1consulting/scolta-laravel/pull/126) and [scolta-wp#207](https://github.com/tag1consulting/scolta-wp/pull/207). **One cross-repository rule survives and is stated as the only one**: a stable release must not depend on an unreleased upstream, enforced by each repository's release lock guard rather than by matching numbers. Self-consistency stays too, since it was never lockstep: the `coherence` check refuses a package that states two different development lines about its own version. Two further corrections in the same pass, both places where the document described a state CI rejects: the version-location table said scolta-drupal and scolta-wp declare a `version` in `composer.json`, which both of them hard-fail on, because the drupal.org Composer facade honours a declared version and a site tracking a dev branch could then `composer update` but never `composer install`; and the table implied those files are the version of record for a release, where the tag is, with drupal.org's packager writing the tag into `scolta.info.yml` at packaging time.
