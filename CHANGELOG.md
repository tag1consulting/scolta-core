...
### Fixed
- **`sanitize_query` no longer logs IPv6 addresses or unhyphenated SSNs in the clear.** The SSN pattern was hyphen-only (`\b\d{3}-\d{2}-\d{4}\b`) and the IP pattern was IPv4-only, so `123 45 6789`, `123456789`, `2001:0db8:85a3:0000:0000:8a2e:0370:7334` and `fe80::1` all passed through untouched into analytics logs — in a function documented as redacting SSN and IP, and relied on as the only sanitization layer by the PHP, WordPress, Drupal, Laravel and 11ty consumers. SSN is now `\b\d{3}[-\s]?\d{2}[-\s]?\d{4}\b`, covering hyphenated, space-separated, mixed and bare nine-digit forms; a new IPv6 pattern runs under the existing `redact_ip` flag alongside the IPv4 one. The `\b` anchors keep the widened SSN pattern off longer digit runs, so a 10-digit phone number and a 16-digit card still redact as `[PHONE]` and `[CC]`. The IPv6 uncompressed branch requires exactly eight groups (the only valid uncompressed length) rather than the looser 3-to-8 used by the tag1.com defense-in-depth layer, so `12:34:56` and MAC addresses are not redacted as `[IP]`; leading-`::` forms (`::1`) are deliberately unmatched because a branch for them cannot carry a leading `\b` and would eat `Path::add`-style namespace syntax in developer-documentation queries. No config, signature or `WASM_INTERFACE_VERSION` change. ([#53](https://github.com/tag1consulting/scolta-core/issues/53)) **[parity]** Two behavior changes for existing sites: any bare nine-digit number in a query now logs as `[SSN]`, and IPv6 hosts now log as `[IP]`. Adapters ship a vendored WASM build, so the fix reaches sites only after a scolta-core release and re-vendor; consumers running their own regex pass on top (tag1.com-11ty-website#1292) can keep it as defense-in-depth.
