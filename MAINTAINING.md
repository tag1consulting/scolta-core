...
| **overlay** | A temporary swap the harness performs on a demo so a run measures the code it says it measures: the demo's committed adapter copy is moved aside and the version under test is installed in its place, then restored. Demos with no overlay are not measured. |
